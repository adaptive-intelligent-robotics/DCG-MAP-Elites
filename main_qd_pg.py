...
class Config:
    ...
    env_batch_size: int
    ga_batch_size: int
    quality_pg_batch_size: int
    diversity_pg_batch_size: int
    ...
    num_q_critic_training_steps: int
    num_d_critic_training_steps: int
    ...
    policy_delay: int
    archive_acceptance_threshold: float
    archive_max_size: int

    num_nearest_neighb: int
    novelty_scaling_ratio: float

@hydra.main(config_path="configs/", config_name="qd_pg")
def main(config: Config) -> None:
    ...
    qpg_emitter_config = QualityPGConfig(
        env_batch_size=config.quality_pg_batch_size,
        ...
        num_critic_training_steps=config.num_q_critic_training_steps,
    ...
    dpg_emitter_config = DiversityPGConfig(
        env_batch_size=config.diversity_pg_batch_size,
        batch_size=config.transitions_batch_size,
        critic_hidden_layer_size=config.critic_hidden_layer_size,
        critic_learning_rate=config.critic_learning_rate,
        actor_learning_rate=config.greedy_learning_rate,
        ...
        replay_buffer_size=config.replay_buffer_size,
        soft_tau_update=config.soft_tau_update,
        num_critic_training_steps=config.num_d_critic_training_steps,
        num_pg_training_steps=config.num_pg_training_steps,
        policy_delay=config.policy_delay,
        archive_acceptance_threshold=config.archive_acceptance_threshold,
    ...
    qdpg_emitter_config = QDPGEmitterConfig(
        ...
        ga_batch_size=config.ga_batch_size,
    )
    ...
    score_novelty = jax.jit(
        functools.partial(
            ...
            num_nearest_neighb=config.num_nearest_neighb,
            scaling_ratio=config.novelty_scaling_ratio,
        )
    )
