...
def make_td3_ablation_3_loss_fn(
    policy_fn: Callable[[Params, Observation], jnp.ndarray],
    policy_dc_fn: Callable[[Params, Observation], jnp.ndarray],
    critic_dc_fn: Callable[[Params, Observation, Action], jnp.ndarray],
    ...
    Args:
        ...
        policy_dc_fn: forward pass through the neural network defining the
            descriptor-conditioned policy.
        critic_dc_fn: forward pass through the neural network defining the
    ...
    def _policy_loss_fn(
        ...
        critic_dc_params: Params,
        transitions: Transition,
    ) -> jnp.ndarray:
        """Policy loss function for TD3 agent"""

        action = policy_fn(policy_params, transitions.obs)
        q_value = critic_dc_fn(
            # type: ignore
            critic_dc_params, obs=jnp.concatenate([transitions.obs, transitions.input_desc], axis=1), actions=action
        )
    ...
    @jax.jit
    def _policy_dc_loss_fn(
        policy_dc_params: Params,
        critic_dc_params: Params,
        transitions: Transition,
    ) -> jnp.ndarray:
        """Descriptor-conditioned policy loss function for TD3 agent"""

        action = policy_dc_fn(policy_dc_params, transitions.obs)
        q_value = critic_dc_fn(
            # type: ignore
            critic_dc_params, obs=jnp.concatenate([transitions.obs, transitions.input_desc], axis=1), actions=action
        )
    ...
    @jax.jit
    def _critic_dc_loss_fn(
        critic_dc_params: Params,
        target_policy_dc_params: Params,
        target_critic_dc_params: Params,
        transitions: Transition,
        random_key: RNGKey,
    ) -> jnp.ndarray:
        ...
        next_action = (
            policy_dc_fn(target_policy_dc_params, transitions.next_obs) + noise
        ).clip(-1.0, 1.0)
        next_q = critic_dc_fn(  # type: ignore
            target_critic_dc_params, obs=jnp.concatenate([transitions.next_obs, transitions.input_desc], axis=1), actions=next_action
        )
        ...
        q_old_action = critic_dc_fn(  # type: ignore
            critic_dc_params,
            obs=jnp.concatenate(
                [transitions.obs, transitions.input_desc], axis=1),
    ...
    return _policy_loss_fn, _policy_dc_loss_fn, _critic_dc_loss_fn
