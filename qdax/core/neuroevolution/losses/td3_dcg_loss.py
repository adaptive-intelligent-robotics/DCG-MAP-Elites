...
def make_td3_dcg_loss_fn(
    ...
    def _policy_dc_loss_fn(
        ...
        q_value = critic_dc_fn(
            critic_dc_params, obs=transitions.obs, actions=action  # type: ignore
        )
        q1_action = jnp.take(q_value, jnp.asarray([0]), axis=-1)
        policy_loss = -jnp.mean(q1_action)
        return policy_loss
    ...
    def _critic_dc_loss_fn(
        ...
        next_q = critic_dc_fn(  # type: ignore
            target_critic_dc_params, obs=transitions.next_obs, actions=next_action
        )
