...
def generate_unroll_dcg(
    ...
    Args:
        ...
        descriptor: descriptor the policy attempts to achieve.
        random_key: random key for stochasiticity handling.
        episode_length: length of the rollout.
        play_step_fn: function describing how a step need to be taken.
    ...
    def _scan_play_step_fn(
        ...
        env_state, policy_params, descriptor, random_key, transitions = play_step_fn(*carry)
        return (env_state, policy_params, descriptor, random_key), transitions

    (state, _, _, _), transitions = jax.lax.scan(
        _scan_play_step_fn,
        (init_state, policy_dc_params, descriptor, random_key),
        (),
        length=episode_length,
    )
    return state, transitions
