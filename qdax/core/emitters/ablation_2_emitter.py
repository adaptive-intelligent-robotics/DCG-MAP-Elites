...
def sample_descriptors_flip(transitions):
    """Simple strategy to sample descriptors where descriptors are flipped.

    Args:
        transitions: batch of transitions.

    Returns:
        batch of sampled descriptors.
    """
    return jnp.flip(transitions.input_desc, axis=0)
...
class Ablation2Emitter(Emitter):
    """
    ...
    def _train_critics(self, emitter_state: DCGMEEmitterState) -> DCGMEEmitterState:
        ...
        samples, random_key = replay_buffer.sample(
            random_key, sample_size=self._config.batch_size // 2
        )

        # Create negative transitions
        negative_input_desc = sample_descriptors_flip(samples)
        negative_samples = samples.replace(obs=jnp.concatenate([samples.obs, negative_input_desc], axis=1),
                                           next_obs=jnp.concatenate(
                                               [samples.next_obs, negative_input_desc], axis=1),
                                           rewards=similarity(
                                               negative_input_desc, samples.desc, self._config.lengthscale)*samples.rewards,
                                           input_desc=negative_input_desc)

        # Create positive transitions (rewards scaling = 1.)
        positive_samples = samples.replace(obs=jnp.concatenate([samples.obs, samples.input_desc], axis=1),
                                           next_obs=jnp.concatenate([samples.next_obs, samples.input_desc], axis=1))

        samples = samples.replace(obs=jnp.concatenate([positive_samples.obs, negative_samples.obs]),
                                  next_obs=jnp.concatenate(
                                      [positive_samples.next_obs, negative_samples.next_obs]),
                                  rewards=jnp.concatenate(
                                      [positive_samples.rewards, negative_samples.rewards]),
                                  dones=jnp.concatenate(
                                      [positive_samples.dones, negative_samples.dones]),
                                  truncations=jnp.concatenate(
                                      [positive_samples.truncations, negative_samples.truncations]),
                                  actions=jnp.concatenate(
                                      [positive_samples.actions, negative_samples.actions]),
                                  state_desc=jnp.concatenate(
                                      [positive_samples.state_desc, negative_samples.state_desc]),
                                  next_state_desc=jnp.concatenate(
                                      [positive_samples.next_state_desc, negative_samples.next_state_desc]),
                                  desc=jnp.concatenate(
                                      [positive_samples.desc, negative_samples.desc]),
                                  input_desc=jnp.concatenate([positive_samples.input_desc, negative_samples.input_desc]))
